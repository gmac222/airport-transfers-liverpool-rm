...
https.get(`https://gmac222.app.n8n.cloud/api/v1/workflows/${WORKFLOW_ID}`, options, (res) => {
  ...
  res.on('end', () => {
    ...
    if (resendNode) {
      // Replace bookings@airporttaxitransfersliverpool.co.uk with onboarding@resend.dev
      resendNode.parameters.jsonBody = resendNode.parameters.jsonBody.replace('bookings@airporttaxitransfersliverpool.co.uk', 'onboarding@resend.dev');
      ...
      const req = https.request(`https://gmac222.app.n8n.cloud/api/v1/workflows/${WORKFLOW_ID}`, putOptions, (putRes) => {
        ...
        putRes.on('end', () => console.log("Changed to onboarding@resend.dev! Status:", putRes.statusCode));
      });
      req.write(putData);
      req.end();
    }
